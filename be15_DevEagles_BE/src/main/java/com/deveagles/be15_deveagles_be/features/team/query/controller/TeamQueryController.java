...
public class TeamQueryController {
  ...
  @Operation(
      summary = "팀원 목록 조회 (감정 분석 포함)",
      description = "내가 소속되어 있는 팀의 팀원 목록과 각 팀원의 최근 감정 분석 결과를 조회합니다.")
  @GetMapping("/{teamId}/members/with-mood")
  public ResponseEntity<ApiResponse<List<TeamMemberResponse>>> getTeamMembersWithMood(
      @AuthenticationPrincipal CustomUser customUser, @PathVariable Long teamId) {

    Long userId = customUser.getUserId(); // 로그인한 사용자 ID

    List<TeamMemberResponse> members = teamQueryService.getTeamMembersWithMood(userId, teamId);
    return ResponseEntity.ok(ApiResponse.success(members));
  }
